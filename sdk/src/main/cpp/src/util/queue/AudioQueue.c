...
//		pthread_mutex_unlock(&(queue->mutex));
		...
		queue->rear = 0;
		queue->more = true;
		__android_log_print(ANDROID_LOG_DEBUG, "audio_queue", "half queue data reached, left=%d,dataLen = %d,capacity = %d", queueLeft,dataLen,queue->capacity);
...
//		queue->rear = 0;
//		queue->more = true;
//		__android_log_print(ANDROID_LOG_DEBUG, "audio_queue", "unenough queue space, left=%d,dataLen = %d,capacity = %d", queueLeft,dataLen,queue->capacity);
//
//		return false;
//	}
